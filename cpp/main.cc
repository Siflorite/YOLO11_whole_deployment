...
torch::Tensor non_max_suppression(torch::Tensor& prediction, float conf_thres = 0.35, float iou_thres = 0.45, int max_det = 300) {
...
void predict(cv::Mat image, torch::jit::script::Module yolo_model, torch::Device device, bool bDraw = false)
{
    try {
        // Load image and preprocess
        // Original Image Process Part
        // cv::Mat image = cv::imread("F:/YOLO/yolo11/cpp/sampleimg170.jpg");
        ...
        std::cout << "Preprocess Time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / 1000.f << "ms, ";
        ...
        std::cout << "NMS Time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / 1000.f << "ms." << std::endl;
        ...
        for (int i = 0; i < keep.size(0); i++) {
            ...
            std::cout << "Rect: [" << x1 << "," << y1 << "," << x2 << "," << y2 << "]  Conf: " << conf << "  Class: " << droplet_classes[cls] << std::endl;
...
int main() {
    ...
    */
    try {
        ...
        // Self-made Video Process Part
        bool bSaveProcessed = true; // True if save processed video
        ...
        while (1)
        {
            ...
            std::cout << "Frame [" << i << "]:";
            predict(frame, yolo_model, device, true);
            if (bSaveProcessed)
            {
